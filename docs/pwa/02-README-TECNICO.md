...
- **Logout offline** (cerrado, MED-1): `logout()` limpia la sesión local SIEMPRE; si el `POST /auth/logout` no llega (offline), se marca un flag no sensible `auth:pendingLogout` en localStorage. En la siguiente apertura, `restore()` NO reabre la sesión y **completa la revocación server-side** de la cookie de refresh al reconectar (reintenta hasta confirmar). Un `login` exitoso limpia el flag. Así un logout sin conexión en un dispositivo compartido/perdido no revive la sesión al reabrir online.
